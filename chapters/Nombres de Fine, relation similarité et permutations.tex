...
Dans toute la suite, on note par $S_{n}$ l'ensemble des permutations de [n]. Soit $m\leq n$ et $x =
	x_{1}x_{2}\cdots x_{m} $ une permutation qui n'est pas nécessairement un élément de $ S_{m} $. La
permutation $st(x) = \sigma_{1}\sigma_{2}\cdots \sigma_{m}$ est obtenu à partir de x en remplaçant
...
motif $\alpha \in S_{m}$, s'il existe $i_{1},i_{2}, \cdots, i_{m}$ tel que $i_{1}<i_{2}< \cdots
	<i_{m}$ et $st(\pi_{i_{1}}\pi_{i_{2}}\cdots \pi_{i_{m}})=\alpha$. \\
Soit $i\in [n]$. $i$ est
un point fixe de $\pi$ si $\pi_{i}=i$.\\Dans toute la suite, on note par  $S_{n}(\alpha)$ l'ensemble
de toutes les permutations $\pi \in S_{n}$ qui ne contiennent pas le motif $\alpha$ et $|S_{n}(\alpha)|:= s_{n}(\alpha)$. $S_{n}^k(\alpha)$ est l'ensemble des permutations $\pi \in
	S_{n}(\alpha)$ qui ont exactement k points fixes et $| S_{n}^{k}(\alpha) |:=s_{n}^{k}(\alpha)$.
\vspace{5pt}

L'objet de ce chapitre est d'étudier et de trouver la relation entre les permutations de $[n]$ sans
le motif $\alpha \in \{132, 321\}$ et le nombre $F_{n}$. L'introduction de la notion des relations
de similarité nous permet de trouver l'expression $s_{n}^{0}(321) = F_{n}$.

\section{Relations de similarité $\mathcal{R}$}
%Dans toute la suite la relation $\mathcal{R}$ est définie sur l'ensemble $[n]$
\begin{definition}
	\begin{rm}
		Une relation de similarité sur [n] est une relation binaire, réflexive et symétrique
...
\begin{definition}
	\begin{rm}
		Soit $i \in [n]$. On dit que i est un point isolé de $\mathcal{R}$, si $\forall j \in [n], (i\mathcal{R}
			j\implies i=j) $
	\end{rm}
\end{definition}
\begin{definition}
	...
	\end{rm}
\end{definition}
Dans toute la suite, on note par $ \mathcal{SR}_{n} $ l'ensemble des relations de similarité sur
$ [n] $ et par $\mathcal{SR}_{n}(k)$ l'ensemble des relations de similarité ayant exactement $k$ points isolés.
On pose par $Sim_{n}$ l'ensemble des n-uples $(r_{1},\cdots, r_{n})$ d'entiers tel que $\forall i\leq n,
	0\leq r_{i} \leq i-1 \text{ et }  r_{i+1}\leq r_{i}+1 \text{ si } i<n $
\newpage
...
% Dans toute la suite on pose $\overline{Sim_{n}} = \{ r\in Sim_{n}; \forall i\leq n-1,(r_{i}=0  \implies r_{i+1}\neq 0) \} $
...
\begin{corollaire}\label{SimNOfK} \text{ }\\
	On a $\#\mathcal{SR}_{n}(k) = \# \{p \in \text{Dyck}(n); |\{i \in \text{Mont p}; \gamma_{i-1}=0 \text{ et } p_{i+1}=d\}|=k\}$.
\end{corollaire}
\textit{Preuve}: En utilisant le \Cref{sr_eq_sim} et la \Cref{phi_tsfm}, on a le résultat.

\begin{corollaire}
	On a:  $|\mathcal{SR}_{n}|=C_{n}$.
\end{corollaire}
...
\begin{corollaire} \label{fnToSRn0}
	\begin{rm}
		On a, $F_{n}=|\mathcal{SR}_{n}(0)|$
	\end{rm}
\end{corollaire}
\textit{Preuve}: En utilisant la \Cref{bij-DyckBar} et le \Cref{SimNOfK} , on obtient le résultat.


\subsection{Relation entre $S_{n}^{k}(321)$ et $\mathcal{SR}_{n}(k)$}
\begin{definition}
	\begin{rm}
...
\begin{corollaire} \label{avoiding_321_and_SR_n}
	$\forall n\geq 0$, $s_{n}^{k}(321)=\#\mathcal{SR}_{n}(k)$
\end{corollaire}
\textit{Preuve}:
...
\end{corollaire}
\textit{Preuve}:
En utilisant le \Cref{avoiding_321_and_SR_n} et \Cref{fnToSRn0}, on a le résultat.
...
\begin{lemme}
	La transformation d'un chemin de \text{Dyck} en un mot de \text{Catalan} précédente est un application bijective
	% Il existe une bijection $\chi$ de $\textit{Cat(n)}$ sur $\text{Dyck}(n)$.
\end{lemme}
...
% \end{proposition}
% 	\underline{\textit{Démonstration}}: On en déduit de la \Cref{bij-DyckBar} (par la transformation de $\overline{\text{Dyck}}(n)$).
...
\end{corollaire}
\textit{Preuve}: \\ En utilisant la \Cref{bijCatN}, on a $s_{n}^{k}(321) = \#\{ c\in Cat(n); | \{i; c_{i}=i \text{ et } c_{i+1}=i+1\} |=k \}$ et en utilisant le \Cref{fn_and_avoiding_321_without_fix}, on a le résultat. $\blacksquare$\vspace{10pt}\\
...
\end{proposition}
Preuve: Soit $c\in Cat(n)$ et $\alpha(c):=\pi \in S_{n}(321)$ où $\alpha$ est la bijection définit dans la \Cref{bijCatN}. On a $fix(\pi) = \# \{i; c_{i}=i \text{ et } c_{i+1}=i+1\}=\#D(c)$. D'où le résultat.\vspace{5pt}\\
...
\]
% On note que si $i=1 \text{ ou } i=2$ alors $Cat_{i}(n)=\emptyset$. \vspace{5pt}\\
...
de $M_{j+1}$. Dans toute la suite, on note par $D(M_{k})$ et  $P(M_{k})$ la dernière lettre et la première
lettres de tout mots $M_{k}$. Soit $j \leq u$ et i une lettre de $M_{j}$. On a:\\
$$
	\begin{cases}
...
\text{ }\vspace{5pt}\\
Preuve:
En utilisant la \Cref{phiEqCOfT}, on a:\\
\[
	\begin{array}{r c l}
...
\end{rm}
\text{}\vspace{10pt}\\
En utilisant la \Cref{firstDefLab} et d'après le Théorème 2.2 de \cite{Desantis}, on le lemme suivant.
\begin{lemme}
...
Montrons qu'elle est bijective. Soit $\pi' \in \mathcal{E}_{n, -r}$ et $\tau\in S_{n}(132)$ définie par $\tau(i) = \tau'(i)+r$. $\pi$ est obtenue par $\pi(i)=\tau^{-1}(i) + r$. Comme $\tau\in S_{n}(132)$, donc d'après le \Cref{l100} on a $\tau^{-1}\in S_{n}(132)$; on en déduit que $\pi \in \mathcal{E}_{n, r}$. Ce qui prouve que $\rho$ est bijective.\vspace{10pt}\\
...
% 		De plus, soit $q\geq i+1$ tel que $\pi_{q} = q$ s'il existe. Alors, il existe p tel que i+p = q. D'où $\pi''_{p} = \pi_{i+p}-i = i+p-i=p$. Cela implique que $fix(\pi'') = fix(\pi_{i+1}\cdots \pi_{n})$.\\
% 		D'abord, nous allons considerer le cas où $r=0$. On a:
% \[
% 	E_{n,0}(x) =\underset{i=1}{\overset{n}{\sum}}\text{ }\underset{\underset{\pi_{i}=n}{\pi \in \mathcal{E}_{n,0}}}{\sum}x^{fix(\pi)} = \underset{i=1}{\overset{n-1}{\sum}}\text{ }\underset{\underset{\pi_{i}=n}{\pi \in \mathcal{E}_{n,0}}}{\sum}x^{fix(\pi)} + \underset{\underset{\pi_{n}=n}{\pi \in \mathcal{E}_{n,0}}}{\sum}x^{fix(\pi)}
% \]
% On en déduit que, $\forall i \in [n-1], fix(\pi) = fix(\pi') + fix(\pi'')$\\
% D'où
% \[
% 	\begin{array} {l l l}
% 		E_{n,0}(x) & = & \underset{i=1}{\overset{n-1}{\sum}} \text{ } \underset{(\pi', \pi'') \in \mathcal{E}_{i-1,n-i}\times \mathcal{E}_{n-i,-i}}{\sum}x^{fix(\pi') + fix(\pi'')} + \underset{\pi \in \mathcal{E}_{n-1,0}}{\sum}x^{fix(\pi)+1} \\
...
% 	\end{array}
% \]
% Enfin, si $r\neq 0$, on a: \[E_{n,r} =\underset{i=1}{\overset{n}{\sum}}\text{ }\underset{\underset{\pi_{i}=n+r}{\pi \in \mathcal{E}_{n,r}}}{\sum}x^{fix(\pi)} \]
% Or $\forall r>0, fix(\pi) = fix(\pi') + fix(\pi'')$\\
...
Preuve:
En utilisant la \Cref{genFn} et pour $k=1$, on a\[ 2F_{n}^{1}+F_{n-1}^{1}=F_{n}^{0}+2F_{n-1}^{0} \]
