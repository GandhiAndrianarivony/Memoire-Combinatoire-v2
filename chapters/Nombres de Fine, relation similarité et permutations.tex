...
\end{lemme}
Preuve:
Montrons tout d'abord que, s'il existe $k\in [s]$ tel que $|w_{k}|>1$, alors $w_{k}$ est une lettre en mots décroissants. Il est évident que c'est le cas car sinon on aura $w_{k}u_{k}$ contient le motif $123$.\\
...
Enfin, soit $\pi \in S_{n}(123)$ et soit $j \in D(\pi)$. On en déduit de la \Cref{SKNBij} et \\ du \Cref{jPtFix} que toute lettre à droite de $j$ et plus petite que $j$ et toute lettre à gauche de $j$ et plus grande que $j$. Cela implique que $j$ est un ssd de $\pi$\\ ou encore 
