...
\begin{definition}
	\begin{rm}
		...
		isolé. Le nombre de relations de similarité non-singulières sur $[n]$ est appelé nombre de Fine d'ordre $n$.
	\end{rm}
\end{definition}
% \begin{definition}
% 	\begin{rm}
% 		Le nombre de Fine d'ordre $n$ est égal au nombre des relations de similarité non-singulières sur  $[n]$.
% 	\end{rm}
% \end{definition}
...
\begin{corollaire} \label{fnToSRn0}
	\begin{rm}
		...
		Le nombre de Fine d'ordre $n$ est  égal au nombre $F_{n}$ des chemins de Fine de longueur $n$
	\end{rm}
\end{corollaire}
Preuve: En utilisant la \Cref{bij-DyckBar} et le \Cref{SimNOfK} , on obtient le résultat. $\blacksquare$\vspace{5pt}\\
% Remarque: D'après le \Cref{fnToSRn0}, le nombre de Fine d'ordre $n$ n'est autre que le nombre $F_{n}$.
Grâce à ce corollaire, nous gardons la notation $F_{n}$ pour désigner le nombre de Fine sur $n$.
...
\]
% On note que si $i=1 \text{ ou } i=2$ alors $\begin{rm}Cat\end{rm}_{i}(n)=\varnothing$. \vspace{5pt}\\
...
\end{proposition}
Preuve: Dans la \Cref{fnbnk}, on a $F_{n} = \sum\limits_{j=0}^{n}(-1)^{j}b(n, j)$. Rappelons que, pour tout $n\geq k$, $c_{n, k} = b(n, k)$. Cela implique que $F_{n}=\sum\limits_{j=0}^{n}(-1)^{j}c_{n, n-j}$. Nous allons distinguer le cas où $n$ est pair et $n$ impair.\vspace{5pt}\\
