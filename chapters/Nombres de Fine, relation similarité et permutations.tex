...
\end{corollaire}
\textit{Preuve}: On obtient le résultat  en utilisant le \Cref{sr_eq_sim} et la \Cref{phi_tsfm} $\blacksquare$.
...
\begin{definition}
	\begin{rm}
		Soit $\pi$ une permutation de $[n]$. On appelle ssd-décomposition de $\pi$ l'expression $\pi = w_{1}u_{1}w_{2}u_2 \cdots w_{s}u_{s}$ où les $u_{i}$ sont les ssd de $\pi$ et les $w_{i}$ sont des sous-mots(éventuellement vide)  de $\pi$.
	\end{rm}
\end{definition}
\begin{lemme}
